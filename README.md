# StyleNest — Microservices E‑Commerce Platform

StyleNest is microservices application for an e‑commerce domain. It combines Spring Boot (Java) and .NET (C#) backend services behind a Spring Cloud Gateway, and an Angular + PrimeNG frontend. The system demonstrates service discovery, centralized config, event‑driven communication (RabbitMQ), and resilience patterns.

## Architecture Overview
- API Gateway (Spring Cloud Gateway)
  - Entry point at `:8085`; validates JWT, forwards user context as `X-USER-ID`, and routes to downstream services.
  - Global CORS (dev‑friendly), circuit breakers, Swagger aggregation.
- Service Discovery (Eureka)
  - Registry/UI at `:8761`; services register and are addressed by logical names.
- Config Server (Spring Cloud Config)
  - Centralized config backed by the `config-repo` folder; services consume on startup.
- Domain Services
  - AuthService (Spring Boot): Register/Login, issues JWT including userId and role.
  - ProductService (Spring Boot): Products and Categories APIs; stock reserve/release; listens to order events.
  - OrderService (Spring Boot): Creates orders, manages status, integrates with stock and payment events.
  - CartService (.NET): Cart per user with TTL; endpoints to add/update/remove/clear items.
  - PaymentService (.NET): Processes payments, publishes events; drives order confirmation.
- Messaging (RabbitMQ)
  - Order/stock/payment events; enables loose coupling and eventual consistency.
- Frontend (Angular + PrimeNG)
  - SPA at `:4200`; dev proxy forwards `/auth` and `/api` calls to API Gateway.

## Key Features
- JWT authentication, user context propagation via the gateway
- Product catalog with categories
- Cart with TTL and bulk operations
- Order creation and payment confirmation flow
- Stock reservation workflow via events
- Resilience (circuit breakers) and optional monitoring (Prometheus + Grafana)

---

## Getting Started

This section mirrors docs/GETTING_STARTED.md for convenience.
...
docker compose up -d
```
if something not working specially the cart just enter this command and try again:
```powershell
...
- The dev proxy forwards API calls to the Gateway at http://localhost:8085:
  - `/auth/*` → http://localhost:8085
  - `/api/*` → http://localhost:8085

Troubleshooting (frontend)
- 403/Unknown error on `/auth/*`: ensure you used `npm start` (proxy enabled) and the API Gateway is running on 8085.
- 504/timeout to `http://localhost:4200/...`: restart with `npm start` so requests go through the proxy instead of the browser hitting 4200 directly.
- After payment, cart should clear automatically and orders page shows the sum of item quantities. If not, hard-refresh the browser.

Production build (optional):
```powershell
npm run build
```
Output is generated under `stylenest-frontend/dist/stylenest-frontend/browser` and can be served by any static web server or a reverse proxy in front of the API Gateway.
...
and then enter this:
```powershell
docker compose up -d --no-deps --build orderservice
```
Open UIs:
- Prometheus: http://localhost:9090
- Grafana: http://localhost:3000 (admin/admin)

be Noticed that the effect in grafana will takes some time after paying and making the order so refresh until it appears
