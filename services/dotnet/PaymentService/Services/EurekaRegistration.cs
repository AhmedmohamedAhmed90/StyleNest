...
public class EurekaOptions
{
    ...
    public string AppName { get; set; } = "PaymentService";
...
public class EurekaRegistration : IHostedService, IDisposable
{
    ...
    public EurekaRegistration(IHttpClientFactory httpFactory, ILogger<EurekaRegistration> logger, IConfiguration cfg)
    {
        ...
        _logger = logger;
        var zone = cfg["eureka:client:serviceUrl:defaultZone"] ?? "http://localhost:8761/eureka/";
    ...
    private async Task RegisterAsync(CancellationToken ct)
    {
        try
        {
            ...
            var host = Environment.MachineName; var ip = "127.0.0.1";
            var payload = new
            {
            ...
            if (!resp.IsSuccessStatusCode && resp.StatusCode != System.Net.HttpStatusCode.NoContent)
                ...
                _logger.LogInformation("Registered {App} with Eureka as {Id}", _opt.AppName, _instanceId);
        }
        catch (Exception ex)
        {
